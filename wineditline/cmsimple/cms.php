...
function property($n) {
	global $cf;
	if ($cf['property'][$n] != '')return tag('meta property="'.$n.'" content="'.$cf['property'][$n].'"')."\n";
}
...
function hide($i) {
	session_start();
	global $c, $edit, $adm;
	return (!($edit && $adm) && (cmscript('hide', $c[$i]) || (cmscript('loggedin', $c[$i]) && (!(isset($_SESSION['SESS_auth']) && $_SESSION['SESS_auth'] == true)))));
}
...
function head() {
	...
	foreach($cf['property'] as $i => $k)$t .= property($i);
